...
import { value as defaultSetting } from '$lib/settings';
...
export const actions = {
	default: async ({ request, locals }) => {
		...
		/** @type {string} */
		const secret_key = data.get('secret_key')?.toString() || defaultSetting.secret_key;
		/** @type {string} */
		const model = data.get('model')?.toString() || defaultSetting.model;
    ...
    if (secret_key === '') {
      messages.push({
		...
		const openai = openaiAPI(secret_key, model);
